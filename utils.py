...
def overheads_to_be_paid(user_overheads, spent_on_overheads):
    '''
    takes amounts in cents from the db, subtracts the overheads paid from 
    the fixed user given amount
    '''
    # no need to convert the currency to euros, since the result goes directly 
    #back to db
...
def validate_registration_form(form):
    ...
    if not form["name"] or len(form["name"]) > 50 or len(form["name"]) < 4:
        error_list.append(
            "Name field cannot be empty, less than 4 or more than 50 characters.")
    

    if not form["password"] or len(form["password"]) > 15 or len(form["password"]) < 5:
        error_list.append(
            "Password field cannot be empty, less than 5 or more than 15 characters.")

    try:
        float(form["starting-credit"])
        if not form["starting-credit"]:
            error_list.append(
                "You must enter a starting credit and it must be a number.")
